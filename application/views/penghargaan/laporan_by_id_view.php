...
class PDF extends FPDF
{
	...
	{    
        ...
        //echo $hari[date("w")].", ".date("j")." ".$bulan[date("n")]." ".date("Y");
                ...
                $this->cell(0,8,'LAPORAN PENGHARGAAN SISWA',0,1,'C',0); 
                $this->Ln(8);
                ...
                if (file_exists('/public_html/sibiko/assets/foto'.$key->nis.'.jpg'))
                {
                    $foto = "$key->nis";
                }
                else {
                ...
                $this->Ln(10);
                $this->setFont('Arial','B',12);
                $this->cell(0,5,'B. KETERANGAN PENGHARGAAN',0,0,'L',1);
                $this->Ln(8);
                $this->setFont('Arial','',10);
                $this->cell(70,5,'Tanggal Penghargaan ',0,0,'L',1);
                $this->cell(5,5,':',0,0,'L',1);
                $this->cell(80,5,date('j', strtotime(date($key->tanggal_penghargaan))).' '.$bulan[date('n',strtotime(date($key->tanggal_penghargaan)))].' '.date('Y', strtotime(date($key->tanggal_penghargaan))),0,0,'L',1);
                $this->Ln(5);
                $this->cell(70,5,'Kelas',0,0,'L',1);
                $this->cell(5,5,':',0,0,'L',1);
                $this->cell(80,5,$key->kelas,0,0,'L',1);
                $this->Ln(5);
                $this->cell(70,5,'Deskripsi',0,0,'L',1);
                $this->cell(5,5,':',0,0,'L',1);
                $this->multicell(110,5,$key->deskripsi_penghargaan,0,'L','L',1);
                $this->cell(70,5,'Poin',0,0,'L',1);
                $this->cell(5,5,':',0,0,'L',1);
                $this->cell(110,5,$key->poin_penghargaan,0,'L','L','false');
                $this->Ln(5);
